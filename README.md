...
> ### Recruiter TL;DR
> - **What it is:** a self-updating dashboard over my LeetCode activity — every solved problem, auto-fetched with its difficulty and official topic tags, visualized and gamified (karate-belt progression, pattern coverage, Blind 75 / NeetCode 150 progress).
...
- **Activity heatmap** — a GitHub-style calendar of daily submissions over the past year.
- **Interview-list progress** — live **Blind 75** and **NeetCode 150** completion, each with an expandable checklist of what's left (linked to LeetCode).
...
- **Per-problem controls** — confidence (Solid / Shaky / Forgot), a ⭐ revisit flag, and freeform notes; flagged or "Forgot" problems collect in a **Needs Revisit** list.
...
flowchart TD
    ...
    L1[blind75 / neetcode150 / patterns / companies<br/>bundled reference data] --> D
...
**Why this shape:**
- **Accumulator over a single fetch** — the public API returns only recent accepted submissions, so a one-shot sync would keep losing older history. Persisting a merged JSON back to the repo each run turns an incomplete endpoint into a complete, growing dataset.
...
- **Reference lists as bundled data** — Blind 75, NeetCode 150, the canonical pattern list, and the company map are static data files, matched against solved slugs client-side. No runtime dependency.
...
| CI/CD | **GitHub Actions → Pages** | sync → commit history → build → deploy (daily cron + push) |
...
- **Solve on LeetCode** → the daily sync picks it up; the mosaic, belt, heatmap, list progress, and patterns all update on the next deploy.
- **Steer your prep** — check *Patterns to learn next* for gaps, and *Blind 75 / NeetCode 150* for list progress.
- **Review** — set a problem's confidence, ⭐-flag it, and use the **Needs Revisit** filter to drill your weak spots.
...
src/
  ...
  components/
    ...
    Heatmap.tsx              # daily-activity calendar
    InterviewProgress.tsx    # Blind 75 / NeetCode 150 progress
    PatternsToLearn.tsx      # untouched-pattern gap analysis
    StatsBar / ProblemBoard / ProblemCard / Select / Ticker / SettingsModal
  lib/
    store.ts           # load problems + overrides, merge, localStorage, optional sync
  ...
  data/
    blind75.ts, neetcode150.ts   # interview list slugs
...
- A "due for review" queue with spaced-repetition scheduling.
- Optional more-frequent sync / on-demand refresh button.
